export const PORT = process.env.PORT || 3000;
export const PG_PORT = process.env.PG_PORT || 5432;  // Cambiado
export const PG_HOST = process.env.PG_HOST || "localhost";
export const PG_USER = process.env.PG_USER || "postgres";
export const PG_PASSWORD = process.env.PG_PASSWORD || "xlgricky20131415";
export const PG_DATABASE = process.env.PG_DATABASE || "tasksdb";

export const ORIGIN = process.env.ORIGIN || ["http://localhost:5173", "https://rocuantazucarfront.up.railway.app/login"];